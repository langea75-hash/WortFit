...
function saveSettings(){saveSettingsData({time:timeSelect.value,category:categorySelect.value,dailyGoal:dailyGoal.value,autoSpeak:autoSpeak.checked,favoritesOnly:favoritesOnly.checked});goHome()}
function loadSettingsIntoForm(){const s=loadSettingsData();if(s.time)timeSelect.value=s.time;if(s.category)categorySelect.value=s.category;if(s.dailyGoal)dailyGoal.value=s.dailyGoal;if(typeof s.autoSpeak==="boolean")autoSpeak.checked=s.autoSpeak;if(typeof s.favoritesOnly==="boolean")favoritesOnly.checked=s.favoritesOnly}
...
function nextCard(){
 ...
 categoryBadge.textContent=n.category;wordImage.src=n.image;wordText.textContent=n.word;syllablesText.textContent=n.syllables;exampleText.textContent="Beispiel: "+n.example;
 sentenceInput.value="";wordArea.classList.add("hidden");sentenceArea.classList.add("hidden");exampleText.classList.add("hidden");exampleButton.classList.remove("hidden");aiResult.classList.add("hidden");feedback.classList.add("hidden");
...
function evaluateCurrentSentence(){
 ...
 lastEvaluation=evaluateSentence(currentCard,sentenceInput.value.trim());
 aiStatus.textContent=lastEvaluation.status;aiMessage.textContent=lastEvaluation.message;aiSuggestion.textContent="Vorschlag: "+lastEvaluation.suggestion;
...
function saveResult(){
 if(!currentCard)return;
 const s=sentenceInput.value.trim();if(!lastEvaluation&&s)lastEvaluation=evaluateSentence(currentCard,s);
